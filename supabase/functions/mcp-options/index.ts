...
let tok:string|null=null,tokAt=0;async function token(){if(tok&&Date.now()-tokAt<300000)return tok;const r=await sql`select decrypted_secret from vault.decrypted_secrets where name='jin10_bearer_token' limit 1`;tok=String(r?.[0]?.decrypted_secret||"");tokAt=Date.now();if(!tok)throw new Error("missing gateway token");return tok}
