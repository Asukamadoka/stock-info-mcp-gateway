...
## Restart persistence (pre-release)

The Quick Tunnel itself remains ephemeral. `.devcontainer/devcontainer.json`
runs `scripts/codespace-control-bootstrap.sh` with `postStartCommand` on each
Codespace start. The bootstrap rotates the capability, starts the repository-
tracked fixed-action server, creates a fresh Quick Tunnel, verifies local health,
and registers the new generation through `codespace-control-register`.

Runtime state is stored under gitignored `.codespace-control/`. Supabase keeps
only runtime discovery state in `codespace_control_registry`; GitHub remains the
canonical source. A new generation invalidates the previous active generation.
Named Tunnel is intentionally deferred until after the initial project launch.

The persisted fixed-action surface adds only status and explicitly named cleanup
to the existing release/apply-patch/commit-pr workflow. It still must not expose
