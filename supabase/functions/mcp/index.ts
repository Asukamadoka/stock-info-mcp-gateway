...
Deno.serve(async(req:Request)=>{
 if(req.method==="GET"){const r=await f(CORE,req);const h=new Headers(r.headers);h.set("x-stock-info-router","core-v3+options+htsc");return new Response(r.body,{status:r.status,headers:h})}
 ...
 if(p?.method==="tools/list"){
   const rs=await Promise.all([f(CORE,mk(),body),f(OPTIONS,mk(),body),f(HTSC,mk(),body)]);
   const js=await Promise.all(rs.map(r=>r.json()));
   return new Response(JSON.stringify({jsonrpc:"2.0",id:p?.id,result:{tools:js.flatMap(x=>x?.result?.tools||[])}}),{status:200,headers:{"content-type":"application/json; charset=utf-8","x-stock-info-router":"merged-tools"}})
 }
